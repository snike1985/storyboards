...
{
	pvs_update_setting('dotpay_account', pvs_result( $_POST["account"] ));
	pvs_update_setting('dotpay_password', pvs_result( $_POST["password"] ));
	pvs_update_setting('dotpay_active', (int) @ $_POST["active"] );
...
<b><?php echo (site_url( ) );?>/payment-notification/?payment=dotpay</b>
</p>
...
<input type='text' name='account'  style="width:400px" value="<?php echo $pvs_global_settings["dotpay_account"] ?>">
</div>

<div class='admin_field'>
<span><?php echo pvs_word_lang( "Secret key" )?>:</span>
<input type='text' name='password'  style="width:400px" value="<?php echo $pvs_global_settings["dotpay_password"] ?>">
</div>

<div class='admin_field'>
<span><?php echo pvs_word_lang( "enable" )?>:</span>
<input type='checkbox' name='active' value="1" <?php
	if ( $pvs_global_settings["dotpay_active"] == 1 ) {
		echo ( "checked" );
	}
?>>
</div>
