...
$order_number = 0;
$sql = "select order_number from " . PVS_DB_PREFIX . "printful";
$rs->open( $sql );
if ( ! $rs->eof )
{
			...
			if ( $ds->row["prints"] == 1 and $printful_id != 0 )
			{
				if ( $ds->row["printslab"] != 1 )
				{
					...
					if ( ! $dr->eof )
					{
						...
						$content .= "<div style='margin-bottom:3px'><a href='<?php echo(pvs_plugins_admin_url('prints_pwinty/index.php'));
...
?></b></td>
			...
			<td>
				<?php
			$sql = "select data,printful_id from " . PVS_DB_PREFIX .
			...
			if ( ! $ds->eof )
			{
				...
				echo ( "<div class='gray'><b>printful order ID:</b> " . $ds->row["printful_id"] .
					"</div>" );
			} else
			{
...
<div style="padding:25px 0px 10px 5px;"><?php
	echo ( pvs_paging( $n, $str, $kolvo, $kolvo2, pvs_plugins_admin_url( 'prints_printful/index.php' ),
