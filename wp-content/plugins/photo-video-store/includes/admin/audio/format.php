...
?>
<form method="post">
	<input type="hidden" name="action" value="format_add">
...
<?php
$n = 0;
$sql = "select * from " . PVS_DB_PREFIX . "audio_format order by name";
$rs->open( $sql );
if ( ! $rs->eof )
{
?>
	<form method="post">
	<input type="hidden" name="action" value="format_delete">
...
?>" style="width:250px"></td>
			<td class="tab1"><input type="checkbox" id="m<?php
			echo $rs->row["id"]
?>" name="m<?php
			echo $rs->row["id"]
?>" value="1"></td>
