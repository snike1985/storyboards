...
while ( ! $rs->eof )
{
	$sql = "select * from " . PVS_DB_PREFIX . "audio_types where license=" . $rs->
	...
	while ( ! $dr->eof )
	{
		...
		$sql = "update " . PVS_DB_PREFIX . "audio_types  set title='" . pvs_result( $_POST["title" .
		...
		{
			$sql = "delete from " . PVS_DB_PREFIX . "audio_types where id_parent=" . $dr->
				row["id_parent"];
			$db->execute( $sql );
		}
