...
?>
	...
	function get_imagga(url,field_id,language,default_value) 
	{
		...
		tags = "";
		lang = lang_to_lang[language];

		jQuery.ajax({
			type:'POST',
			url:ajaxurl,
			data:'action=pvs_recognition_imagga&url=' + url + '&lang=' + lang,
	...
	echo ( @$imagga_files );
?>
		for(key in imagga_files) 
		{
			get_imagga(imagga_files[key],key,'<?php
...
?>
	...
	function apply_keywords(from_field, to_field)
	{
		...
		{
			language = $("#recognition_lang").val();
			if(language != site_lang)
			{
				if($("#translate_keywords_"+language))
				{
					translation_add('keywords',site_lang_name[language],site_lang_name2[language],language,'textarea_translation');
				}
				to_field = "translate_keywords_"+language;
			}
		}
