...
$(document).ready(function(){
	$("#currency_add").colorbox({width:"500",height:"", inline:true, href:"#new_box",scrolling:false});
});
</script>
...
<a class="btn btn-success toright" href="" id="currency_add"><i class="icon-envelope icon-white"></i> <?php echo pvs_word_lang( "new message" )?></a>

<div style='display:none'>
		
		<div id='new_box'>
		<div class="modal_header"><?php echo pvs_word_lang( "new message" )?></div>
		
		<form method="post">
			<input type="hidden" name="action" value="send_message">
			<div class="form_field">
				<span><b><?php echo pvs_word_lang( "To user" )?> (login)</b></span>
				<input type="text" name="to" value="" style="width:143px">
			</div>
			...
			<div class="form_field">
				<input class="btn btn-primary" type="submit" value="<?php echo pvs_word_lang( "send" )?>">
...
$sql = "select id_parent,touser,fromuser,subject,content,data from " .
	PVS_DB_PREFIX . "messages where id_parent>0 ";
...
<div style="padding:0px 0px 15px 6px"><?php echo ( pvs_paging( $record_count, $str, $kolvo, $kolvo2, pvs_plugins_admin_url('messages/index.php'), "&" . $var_search .
		$var_sort ) );
?></div>
...
<form method="post" action="<?php echo(pvs_plugins_admin_url('messages/index.php'));?>" style="margin:0px"  id="adminform" name="adminform">
...
<a href="<?php echo(pvs_plugins_admin_url('messages/index.php'));?>&<?php echo $var_search
?>&aid=<?php
	if ( $aid == 2 ) {
		echo ( 1 );
	} else {
...
<th class="hidden-phone hidden-tablet"><?php echo pvs_word_lang( "from" )?></th>
<th class="hidden-phone hidden-tablet"><?php echo pvs_word_lang( "to" )?></th>
<th><?php echo pvs_word_lang( "subject" )?></th>
...
<?php
	...
	while ( ! $rs->eof ) {
		$cl3 = "";
		$cl_script = "";
		if ( isset( $_SESSION["user_messages_id"] ) and ! isset( $_SESSION["admin_rows_messages" .
			$rs->row["id_parent"]] ) and $rs->row["id_parent"] > $_SESSION["user_messages_id"] ) {
			$cl3 = "<span class='label label-danger messages" . $rs->row["id_parent"] . "'>" . pvs_word_lang("new") . "</span>";
			$cl_script = "onMouseover=\"pvs_deselect_row('messages" . $rs->row["id_parent"] . "')\"";
		}
?>
...
<td><div class="link_message"><?php echo $rs->row["subject"] ?></dov></td>
...
<div style="padding:25px 0px 0px 6px;"><?php echo ( pvs_paging( $record_count, $str, $kolvo, $kolvo2, pvs_plugins_admin_url('messages/index.php'), "&" . $var_search .
		$var_sort ) );
?></div>
<?php
} else
