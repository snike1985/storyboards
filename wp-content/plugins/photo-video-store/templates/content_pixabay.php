...
if ( isset( $pixabay_results ) ) {
	//Show only pixabay syntax
	...
	//var_dump($pixabay_results);

	$pvs_theme_content[ 'id' ] = $pixabay_results->id;

	if ( get_query_var("pixabay_type") == "videos" ) {

		$player_video_id = strval( @$pixabay_results->id );
		...
		$player_video_height = round( $pvs_global_settings["ffmpeg_video_width"] *
			166 / 295 );
		$player_preview_video = @$pixabay_results->videos->
			small->url;
		$player_preview_photo = "https://i.vimeocdn.com/video/" .
			@$pixabay_results->picture_id . "_295x166.jpg";
		
		include( PVS_PATH . 'includes/players/video_player.php');

		$pvs_theme_content[ 'image' ] = $video_player;
		$pvs_theme_content[ 'downloadsample' ] = @$pixabay_results->videos->small->
			url;
		$pvs_theme_content[ 'share_image' ] = urlencode( "https://i.vimeocdn.com/video/" .
			@$pixabay_results->picture_id . "_295x166.jpg" );
	} else {
		$pvs_theme_content[ 'image' ] = "<img src='" . @$pixabay_results->
			webformatURL . "' />";
		$pvs_theme_content[ 'downloadsample' ] = @$pixabay_results->webformatURL;
		$pvs_theme_content[ 'share_image' ] = urlencode( @$pixabay_results->
			webformatURL );
	}

	$publication_type = pvs_result( get_query_var("pixabay_type") );

	$pvs_theme_content[ 'title' ] = @$pixabay_results->tags;
	$pvs_theme_content[ 'keywords' ] = @$pixabay_keywords_links;
	$pvs_theme_content[ 'keywords_lite' ] = @$pixabay_keywords_links;
	$pvs_theme_content[ 'description' ] = "";
	$pvs_theme_content[ 'category' ] = @$pixabay_categories_links;
	$pvs_theme_content[ 'author' ] = '<b>' . pvs_word_lang( "Contributor" ) .
		':</b> ' . @$pixabay_results->user;
	...
	$pvs_theme_content[ 'fotomoto' ] = "<script type='text/javascript' src='//widget.fotomoto.com/stores/script/" . $pvs_global_settings["fotomoto_id"] .
		".js'></script>";
	$pvs_theme_content[ 'share_title' ] = urlencode(@$pixabay_results->tags );

	if ( (int)get_query_var('pvs_print_id') > 0 ) {
		$pvs_theme_content[ 'share_url' ] = urlencode( site_url() . pvs_print_url( @$pixabay_results->
			id, ( int )get_query_var("pvs_print_id"), @$pixabay_results->tags, $prints_preview,
			"pixabay" ) );
	} else {
		$pvs_theme_content[ 'share_url' ] = urlencode( site_url() .
			pvs_get_stock_page_url( "pixabay", @$pixabay_results->id, @$pixabay_results->
			tags, get_query_var("pixabay_type") ) );
	}
	...
	$pvs_theme_content[ 'type' ] = '<a href="' . site_url() .
		'/?stock=pixabay&stock_type=' . $publication_type . '" >' .
	...
	//Duration
	if ( isset( $pixabay_results->duration ) ) {
		$pvs_theme_content[ 'flag_duration' ] = true;
		$pvs_theme_content[ 'duration' ] = @$pixabay_results->duration;
	} else {
	...
	if ( (int)get_query_var('pvs_print_id') == 0 ) {
		...
		if ( $pvs_global_settings["pixabay_prints"] and $pvs_global_settings["pixabay_show"] ==
			2 and get_query_var("pixabay_type") != "audio" and get_query_var("pixabay_type") != "videos" )
		{
			$display_files = 'none';
		...
		if ( $pvs_global_settings["pixabay_files"] and $pvs_global_settings["pixabay_prints"] and
			get_query_var("pixabay_type") != "videos" )
		{
		...
		if ( get_query_var("pixabay_type") != "videos" )
		{
			$sizes .= "<tr><td>" . pvs_word_lang( "photo" ) . "</td><td>" . @$pixabay_results->
				imageWidth . "x" . @$pixabay_results->imageHeight . "px</td></tr>";
		} else
		{
			$sizes .= "<tr><td>" . pvs_word_lang( "video" ) . "</td><td>" . @$pixabay_results->
				videos->large->width . "x" . @$pixabay_results->videos->large->height .
				"px</td></tr>";
		}

		$sizes .= "</table><br>";

		$sizes .= "<a href='" . @$pixabay_results->pageURL .
			"' target='blank' class = 'btn btn-primary btn-lg' style='color:#ffffff;text-decoration:none'>" .
			pvs_word_lang( "Download from Pixabay" ) . "</a></div>";

		if ( $pvs_global_settings["pixabay_prints"] and get_query_var("pixabay_type") !=
			"videos" )
		{
			...
			$sizes .= "</table><br><a href=\"javascript:prints_stock('pixabay'," . @$pixabay_results->
				id . ",'" . urlencode( @$pixabay_results->pageURL ) . "','" . urlencode( @$pixabay_results->
				previewURL ) . "','" . pvs_get_stock_page_url( "pixabay", @$pixabay_results->id,
				@$pixabay_results->tags, get_query_var("pixabay_type") ) . "','" . addslashes( @$pixabay_results->
				tags ) . "')\" class = 'btn btn-danger btn-lg' style='color:#ffffff;text-decoration:none;'>" .
	...
	if ( (int)get_query_var('pvs_print_id') == 0 ) {
		if ( get_query_var("pixabay_type") == "photo" )
		{
			$url = 'https://pixabay.com/api/?key=' . $pvs_global_settings["pixabay_id"] .
				'&q=' . urlencode( str_replace( ",", " ", @$pixabay_results->tags ) );
		} else
		{
		...
		if ( ! curl_errno( $ch ) )
		{
			$pixabay_related = json_decode( $data );
			//var_dump($pixabay_related);

			if ( isset( $pixabay_related->hits ) )
			{
				foreach ( $pixabay_related->hits as $key => $value )
				{
					//Image
					if ( get_query_var("pixabay_type") == "photo" )
					{
						$preview_title = @$value->tags;
						$preview_img = @$value->webformatURL;
					...
					{
						$preview_title = @$value->tags;
						$preview_img = "https://i.vimeocdn.com/video/" . @$value->picture_id .
							"_295x166.jpg";
					...
					$related_description = "";
					$related_url = pvs_get_stock_page_url( "pixabay", @$value->
						id, @$value->tags, get_query_var("pixabay_type") );
	...
	//Prints
	if ( (int)get_query_var('pvs_print_id') > 0 ) {
		$preview_url = @$pixabay_results->webformatURL;
		$iframe_width = @$pixabay_results->webformatWidth;
		$iframe_height = @$pixabay_results->webformatHeight;
		$default_width = @$pixabay_results->imageWidth;
		$default_height = @$pixabay_results->imageHeight;
		...
		$stock_id = @$pixabay_results->id;
		$stock_type = "pixabay";

		$pvs_theme_content[ 'stock_type' ] = $stock_type;
		$pvs_theme_content[ 'stock_id' ] = $stock_id;
		$pvs_theme_content[ 'stock_url' ] = @$pixabay_results->pageURL;
		$pvs_theme_content[ 'stock_preview' ] = $preview_url;
		$pvs_theme_content[ 'stock_site_url' ] = pvs_print_url( @$pixabay_results->
			id, ( int )get_query_var("pvs_print_id"), @$pixabay_results->tags, $prints_preview,
			"pixabay" );

		$id_parent = get_query_var('pixabay');
