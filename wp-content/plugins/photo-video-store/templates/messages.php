...
<h1><?php echo pvs_word_lang( "messages" )?> - <?php echo pvs_word_lang( "inbox" )?></h1>
...
$sql = "select id_parent,touser,fromuser,subject,data,viewed,trash,del from " .
	...
	"' and trash=0 and del=0 order by data desc";
$rs->open( $sql );
if ( ! $rs->eof ) {
?>
	<form method="post" action="<?php echo (site_url( ) );?>/messages-to-trash/">
...
?>
	...
	<?php echo ( pvs_paging( $n, $str, $kolvo, $kolvo2, "<?php echo (site_url( ) );?>/messages/", "" ) );
} else
{
