...
class UploadHandler
{
	...
	function __construct( $options = null, $initialize = true ) {
		...
		global $lphoto;
		$this->options = array(
			'script_url' => site_url() . '/upload-photo-jquery/',
			'upload_dir' => pvs_upload_dir() . "/content/user_" . get_current_user_id() .
				"/",
			'upload_url' => pvs_upload_dir('baseurl') . "/content/user_" . get_current_user_id() .
				"/",
			...
			'inline_file_types' => '/\.(jpe?g|gif|png|raw|tif?f|eps)$/i',
			// Defines which files (based on their names) are accepted for upload:
			'accept_file_types' => '/\.(jpe?g|gif|png|raw|tif?f|eps)$/i',
			// The php.ini settings upload_max_filesize and post_max_size
			// take precedence over the following max_file_size setting:
			'max_file_size' => $lphoto * 1000000000,
			'min_file_size' => 1,
			...
			'image_versions' => array( // Uncomment the following version to restrict the size of
				...
				*/
				'thumbnail' => array( 'max_width' => $pvs_global_settings["thumb_width"],
						'max_height' => $pvs_global_settings["thumb_height"] ) ) );
		if ( $options ) {
	...
	protected function get_full_url() {
		...
		return ( $https ? 'https://' : 'http://' ) . ( ! empty( $_SERVER['REMOTE_USER'] ) ?
			$_SERVER['REMOTE_USER'] . '@' : '' ) . ( isset( $_SERVER['HTTP_HOST'] ) ? $_SERVER['HTTP_HOST'] :
			( $_SERVER['SERVER_NAME'] . ( $https && $_SERVER['SERVER_PORT'] === 443 || $_SERVER['SERVER_PORT']
			=== 80 ? '' : ':' . $_SERVER['SERVER_PORT'] ) ) ) . substr( $_SERVER['SCRIPT_NAME'],
	...
	protected function get_download_url( $file_name, $version = null ) {
		if ( $this->options['download_via_php'] ) {
			$url = $this->options['script_url'] . $this->get_query_separator( $this->
				options['script_url'] ) . 'file=' . rawurlencode( $file_name );
			if ( $version )
			{
	...
	protected function set_file_delete_properties( $file ) {
		$file->delete_url = $this->options['script_url'] . $this->get_query_separator( $this->
			options['script_url'] ) . 'file=' . rawurlencode( $file->name );
	...
	protected function create_scaled_image( $file_name, $version, $options ) {
		...
		switch ( strtolower( substr( strrchr( $file_name, '.' ), 1 ) ) ) {
			...
			case 'jpeg':
				...
				$image_quality = isset( $options['jpeg_quality'] ) ? $options['jpeg_quality'] :
					75;
				break;
			case 'gif':
		...
		$success = $src_img && @imagecopyresampled( $new_img, $src_img, 0, 0, 0, 0, $new_width,
			$new_height, $img_width, $img_height ) && $write_image( $new_img, $new_file_path,
			$image_quality );
	...
	protected function get_error_message( $error ) {
		return array_key_exists( $error, $this->error_messages ) ? $this->
	...
	protected function validate( $uploaded_file, $file, $error, $index ) {
		...
		if ( is_int( $this->options['max_number_of_files'] ) && ( $this->
	...
	protected function trim_file_name( $name, $type, $index, $content_range ) {
		...
		if ( strpos( $name, '.' ) === false && preg_match( '/^image\/(gif|jpe?g|png)/',
			$type, $matches ) ) {
			$name .= '.' . $matches[1];
		}
		return $name;
	...
	protected function get_file_name( $name, $type, $index, $content_range ) {
		return $this->get_unique_filename( $this->trim_file_name( $name, $type, $index,
			$content_range ), $type, $index, $content_range );
	}
	...
	protected function generate_response( $content, $print_response = true ) {
		if ( $print_response ) {
			...
			if ( isset( $_SERVER['HTTP_CONTENT_RANGE'] ) )
			{
				...
				{
					$this->header( 'Range: 0-' . ( $this->fix_integer_overflow( intval( $files[0]->
						size ) ) - 1 ) );
				}
			}
			$this->body( $json );
	...
	public function get( $print_response = true ) {
		...
		$file_name = $this->get_file_name_param();
		if ( $file_name ) {
			$response = array( substr( $this->options['param_name'], 0, -1 ) => $this->
	...
	public function post( $print_response = true ) {
		...
		$file_name = isset( $_SERVER['HTTP_CONTENT_DISPOSITION'] ) ? rawurldecode( preg_replace
			( '/(^[^"]+")|("$)/', '', $_SERVER['HTTP_CONTENT_DISPOSITION'] ) ) : null;
		// Parse the Content-Range header, which has the following form:
		// Content-Range: bytes 0-524287/2000000
		$content_range = isset( $_SERVER['HTTP_CONTENT_RANGE'] ) ? preg_split( '/[^0-9]+/',
		...
		if ( $upload && is_array( $upload['tmp_name'] ) ) {
			...
			{
				$files[] = $this->handle_file_upload( $upload['tmp_name'][$index], $file_name ?
					$file_name : $upload['name'][$index], $size ? $size : $upload['size'][$index], $upload['type'][$index],
			...
			$files[] = $this->handle_file_upload( isset( $upload['tmp_name'] ) ? $upload['tmp_name'] : null,
				...
				isset( $upload['type'] ) ? $upload['type'] : $_SERVER['CONTENT_TYPE'], isset( $upload['error'] ) ?
